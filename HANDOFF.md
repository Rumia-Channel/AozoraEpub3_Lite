...
残差分（約110行）:
- **画像 float/単ページ分類（約41行）**: 注記0002/0005/0008（`fit` vs `width:%`、alt）、画像回り込み28、出版社5
- **0030 注記内注記（12行）**: `「羊羹」は底本では「…」］` 等の入れ子注記の「」］残骸
...
- **0029 2行**: 内側注記を含む外字注記（`〓super（…）`）
- **外字画像 2行**: ルビ基底の画像注記

ブロック注記（字下げ省略のclose+open同Line、行頭空白、ネスト）は対応済み。残りは上記カテゴリ。
