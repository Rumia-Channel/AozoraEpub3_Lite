...
### 2026-09-15 (続き): 残差の解消 (commit `7a5543d`)

前節で「未修正」として挙げた 3 件と、note_coverage に残っていた
`ページ左下` / `ページの左下` の 2 件を解消した。

- **TOC フォールバック**: 章が無くても表題項目 (`TitleToc`) があれば
  nav.xhtml / toc.ncx は `本文` フォールバックを使わない (Java は表題項目で
  `hasNcxItem` が真になる)。表題項目の挿入を章リスト側へ移し、フォールバック
  判定をその後に行う。表題ページを出さない場合は従来どおり `本文`
...
- **複合字下げの閉じ注記の直前の全角スペース**: Java の `ch[idx+1]` は
  ...
  後ろに何も無いものとして扱われ、禁則調整の対象外になる
- **章行の `id="kobo.N.M"`**: 改ページ注記と本文が同じ行にある章行
  (`［＃ページ左下］本文` など) で、タグに kobo ID が入るようにした。
  改ページでセクションを flush すると行番号が変わるため、章行の対応を
  付け替える (`chapter_line_before_flush`)

結果: `tools/note_coverage.py` の差分は 5/589 → **3/589** (残りは Java が
閉じタグを二重出力する `地付き` / `字下げ省略` / `行内地付き` のみ)。
...
- `test_title.txt` 0001: Java は `BookInfo.isImageSectionLine`
  (`checkImageOnly`: 画像単独行 + 直後の改ページ + 2 行前が改ページまたは先頭)
  でその行を noBr にし、さらにタイトル前バッファの flush で `<p><br/></p>` を
  足す。判定は**メタデータ除去前の生の行**を前提にしており、本文だけを
  受け取る `aozora_text_to_xhtml_sections*` では同じ本文でも
  (a) ファイル先頭の画像行と (b) メタデータ行の直後の画像行を区別できない。
  再現には生テキストの行文脈を変換器へ渡す API 追加が必要
- 複合字下げの**同一行クローズ** (`［＃ここから５字下げ］あ　［＃ここで字下げ終わり］`)
  で、直前の行から続く `</div>` が同じ出力行に載る場合、Java はその分も
  `idx` に数えるため末尾の全角スペースが変換される。Lite は行ごとに変換する
  ためキャリーオーバーしていない (合成入力のみで再現)
